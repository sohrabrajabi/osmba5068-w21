...
class BayesianGhostBusterAgent(VacuumAgent):
    ...
    def bust_strategy(self):
		...
		suckyShouldBust = False #  Define this variable -- when should sucky bust, and what square should it bust?
        if suckyShouldBust:  #  Your code here -- when should I try busting?  
            self.log(f"GOING TO BUST at {unbusted[0][0]}, probability is " + "{:.2f}".format(unbusted[0][1]))
            return self.bust_at(unbusted[0][0])
        unprobed = None # Define the variable unprobed here -- the list of unprobed coordinates, sorted in descending order of probability
        if len(unprobed) > 0:
